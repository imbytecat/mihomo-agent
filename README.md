...
插件通过「概览 / 设置 / 日志」切换页面，运行状态、任务进度与取消入口始终可见，切换不会清空草稿。「日志」页可查看当前任务或运行日志、暂停刷新；向上翻阅时暂停跟随，点击「跟随最新」恢复，离开日志页停止轮询。操作失败时自动切到日志页显示具体原因。任务详情显示本地时间的开始、结束与耗时；worker 日志统一包含时间、级别、任务 ID，结束记录包含耗时。CLI 可用 `cancel ID` 请求取消并等待清理完成，`cancel ID --no-wait` 只等待取消请求被接收；取消任务不会停止已经运行的 Mihomo。
