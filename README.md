...
当前协议为 9、数据库 schema 为 7，不提供旧状态迁移。替换插件后若状态不可读取，可直接点击首页「卸载现有安装」；插件会调用设备上的 ctl 停止服务并清理，确认安装目录和引导目录消失后即可重新安装。卸载入口不依赖运行状态解析，失败会显示具体原因，不直接强删目录。
...
插件底部固定提供可展开的「任务与日志」区，可查看当前任务或运行日志、暂停刷新；向上翻阅时暂停跟随，点击「跟随最新」恢复。操作失败时自动展开具体原因。任务详情显示本地时间的开始、结束与耗时；worker 日志统一包含时间、级别、任务 ID，结束记录包含耗时。CLI 可用 `cancel ID` 请求取消并等待清理完成，`cancel ID --no-wait` 只等待取消请求被接收；取消任务不会停止已经运行的 Mihomo。
