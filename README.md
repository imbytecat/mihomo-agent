...
6. 填写返回完整 mihomo YAML 的订阅链接，「保存订阅」→「更新订阅」→「启动」。保存订阅只保存链接，更新订阅才下载并应用配置；服务运行中更新会重启，停止状态更新只保存。
...
镜像地址可在安装服务前填写，只有操作执行期间暂时禁用编辑。「核心下载」中的「保存」只保存镜像；「保存并下载」先保存当前输入，再用于下载最新官方核心。字段显示未保存/已保存，末尾斜杠会归一化。下载失败不会撤销已保存的镜像；保存失败则保持未保存。再次打开页面会从设备读取。
...
订阅、镜像、共享接口各自保存，不再使用一个按钮保存跨区域设置。字段说明保持简短，操作反馈由 Sonner 在视口顶部显示；同一操作的进度复用一条通知，成功自动消失，错误保留更久。自动刷新不弹成功通知，详细结果保留在「操作详情」，日志/诊断和错误会自动展开。Sonner 随插件打包，无额外 CDN 请求。
