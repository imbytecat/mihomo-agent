...
下载失败时，顶部提示显示失败步骤，点「详情」查看请求目标、HTTP 状态或浏览器原始错误、处理建议。`Failed to fetch` 只表示浏览器未获得可读取的响应，不能单凭它区分网络、DNS、TLS 或跨域限制。若是「查询最新版本失败」，请求来自管理浏览器，尚未开始核心文件下载；可在同一浏览器打开详情中的 GitHub API 地址检查。若是「上传到 F50」或设备通信失败，检查热点连接、UFI 登录和高级功能。后台任务启动后通信中断时，任务可能继续运行，应恢复连接后刷新状态，避免重复安装。设备端安装失败会在详情中带出经过脱敏的安装日志。
