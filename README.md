...
2. 从 [Release](https://github.com/imbytecat/ufi-mihomo/releases/latest) 下载 `ufi-mihomo.js`（或自行构建）。UFI 开启高级功能，导入 JS，提交保存并刷新。
...
自动入口识别结合常见 F50 LAN 名称、私有 IPv4 和所有路由表的出口，排除蜂窝、VPN 与上游 Wi-Fi。每 5 秒检查共享网络，接口消失时撤接管并等待，但保留监听端口保护。未知命名可手动覆盖，不会扩大接管到所有接口。
...
设备守护进程管理 mihomo，并调用小型 shell 桥接 Android 网络命令。启动核心前先限制监听端口只允许回环；共享网络就绪后才放行 LAN，防止等待热点时暴露到蜂窝侧。监听检查关联核心 socket inode，进程记录包含 Linux 启动时间以防 PID 复用。规则使用 A/B 链切换；多个表之间不构成内核原子事务。停止/故障恢复期间系统可能直连，**不是断网保护模式**。
...
停止/禁用自启优先使用界面。Root Shell 也可提交相同设备任务，返回任务 ID 后查看结果：

```sh
/data/ufi-mihomo/agent stop
/data/ufi-mihomo/agent job <任务ID>
# 等停止任务完成后再执行：
/data/ufi-mihomo/agent boot-off
```

不要直接删除文件或清空系统防火墙。自动清理任务临时下载和旧配置，只保留当前及最近两个其他配置版本；卸载备份不自动删除。轻量任务记录保留用于防止重复执行。核心/守护日志分别限制到约 1 MiB / 256 KiB。
