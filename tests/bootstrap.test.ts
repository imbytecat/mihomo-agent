...
test('bootstrap verifies bytes before execution and reports failures without losing status', async () => {
  ...
  const id = 'a'.repeat(32), job = join(base, 'jobs', id);
  await mkdir(job, { recursive: true });
  const fixture = join(base, 'fixture'), marker = join(base, 'executed'), curl = join(base, 'curl');
  ...
  const source = (await readFile('src/bootstrap.sh', 'utf8'))
    .replace('BASE=/data/ufi-mihomo-bootstrap', 'BASE=' + quote(base))
    .replace('CURL=/data/data/com.minikano.f50_sms/files/curl', 'CURL=' + quote(curl))
    .replace('umask 077', 'getprop() { echo arm64-v8a; }\numask 077');
  await writeFile(script, source);
  async function run(mode: string, digest = '') {
    const child = Bun.spawn(['sh', script, mode, id, '', 'https://fixture.invalid/agent', digest, '', ''], {
      env: { ...process.env, UFI_TEST_FIXTURE: fixture, UFI_TEST_EXEC_MARK: marker }, stdout: 'pipe', stderr: 'pipe',
    });
  ...
  try {
    ...
    const stale = JSON.stringify({ id, action: 'bootstrap', state: 'running', phase: 'download', updated: '2026-01-01T00:00:00Z' });
