...
try {
  ...
  await check('!!document.querySelector("[data-group=maintenance] [data-action=update-agent]")');
  await check('document.querySelector("[data-action=update-agent]").textContent === "更新" && document.querySelector("[data-group=maintenance] [data-action=download]").textContent === "更新" && document.querySelector("[data-action=download-dashboard]").textContent === "安装"');
  await browser('click', '[data-action=update-agent]');
  await browser('wait', '--fn', 'mockDeviceState.task?.action === "update-agent" && mockDeviceState.task?.state === "succeeded"');
  await idle();
  ...
  await browser('wait', '--fn', 'document.querySelector("[data-version=core]").textContent === "v9.8.7"');
  await idle();
  await check('mockDeviceState.settings.githubProxy === "https://ghfast.top" && mockIntents.filter(x => x.action === "save-github-proxy").length === 1');
  await check('!document.querySelector("[data-task]") && !document.body.innerText.includes("已安装，校验通过") && !!document.querySelector("[data-group=maintenance] [data-install-task]")');

  await browser('fill', '[data-setting=githubProxy]', 'https://first.example');
  await browser('press', 'Tab');
  await browser('wait', '--fn', 'document.querySelector("[data-save-status=githubProxy]")?.textContent.includes("保存中")');
  await browser('fill', '[data-setting=githubProxy]', 'https://second.example');
  await browser('wait', '--fn', 'mockDeviceState.settings.githubProxy === "https://first.example"');
  await check('document.querySelector("[data-setting=githubProxy]").value === "https://second.example"');
  await browser('press', 'Tab');
  await browser('wait', '--fn', 'mockDeviceState.settings.githubProxy === "https://second.example"');
  ...
  await check('mockDeviceState.settings.githubProxy === "https://second.example"');
  await browser('eval', 'window.mockUploadFailure = false');
  await browser('find', 'role', 'button', 'click', '--name', '重试保存', '--exact');
  await browser('wait', '--fn', 'mockDeviceState.settings.githubProxy === "https://retry.example"');
  await browser('fill', '[data-setting=githubProxy]', 'http://invalid.example');
  await browser('click', '[data-group=maintenance] [data-action=download]');
  await idle();
  await check('mockDeviceState.settings.githubProxy === "https://retry.example" && mockIntents.filter(x => x.action === "download").length === 1');

  await page('missing-service');
  await browser('fill', '[data-setting=githubProxy]', 'https://before-install.example');
  await browser('click', '[data-primary=true][data-action=install]');
  await browser('wait', '--fn', 'mockDeviceState.service');
  await idle();
  await check('document.querySelector("[data-setting=githubProxy]").value === "https://before-install.example"');
  await browser('click', '[data-group=maintenance] [data-action=download]');
  await browser('wait', '--fn', 'document.querySelector("[data-version=core]").textContent === "v9.8.7"');
  await idle();
  await check('mockDeviceState.settings.githubProxy === "https://before-install.example"');
  ...
  await browser('wait', '--fn', '!document.querySelector("[data-dialog=uninstall]")');
  ...
  await browser('wait', '--fn', '!mockDeviceState.service');
  await idle();
  await check('!mockDeviceState.running && !mockDeviceState.boot && !mockDeviceState.agent');
  assert.equal(await browser('errors'), '');
  await page('missing-core');
  await check('document.querySelector("[data-group=maintenance] [data-action=download]").textContent === "安装"');
  ...
  await check('mockIntents.length === 0 && document.querySelector("[data-version=core]").textContent === "v9.8.7" && !document.querySelector("[data-task]")');
  await page('running');
  await browser('click', '[data-settings] > summary');
  ...
  await check('document.querySelector("[data-dialog=secret] input").value === "short" && mockCommands.every(c => !c.includes(JSON.stringify({secret: "short"})))');
  await closeModal('关闭密钥');
  await browser('find', 'role', 'button', 'click', '--name', '更多操作', '--exact');
  await browser('find', 'role', 'menuitem', 'click', '--name', '运行日志', '--exact');
  await browser('wait', '[data-dialog=result][data-state=open]');
  await check('document.querySelector("[data-dialog=result] [data-dialog-title]").textContent === "运行日志"');
  await closeModal();
  await browser('find', 'role', 'button', 'click', '--name', '更多操作', '--exact');
  await browser('find', 'role', 'menuitem', 'click', '--name', '最近任务', '--exact');
  await browser('wait', '[data-dialog=result][data-state=open]');
  await check('document.querySelector("[data-dialog=result] [data-dialog-title]").textContent === "任务详情" && document.querySelector("[data-output]").textContent.includes("面板设置") && !document.querySelector("[data-output]").textContent.includes("core.log")');
  await closeModal();
