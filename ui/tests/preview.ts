...
export {};
const mock = await Bun.build({
  entrypoints: ['dev/mock.ts'],
  target: 'browser',
});
if (!mock.success) throw new Error('Cannot build browser mock');
const server = Bun.serve({
  hostname: '127.0.0.1',
  port: Number(process.env.UFI_PREVIEW_PORT ?? 3007),
  fetch(request) {
    const path = new URL(request.url).pathname;
    if (path === '/plugin.js')
      return new Response(Bun.file('dist/mihomo-agent-ufi.js'));
    if (path === '/mock.js') return new Response(mock.outputs[0]);
    return new Response(
      `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>UFI mock</title></head><body>
