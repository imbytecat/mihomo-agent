...
func (a *Agent) dashboard() DashboardStatus {
	...
	if !regularFile(a.runtime("dashboard", "index.html")) {
		return state
	}
	_ = readJSON(a.runtime("dashboard", "ufi-dashboard.json"), &state)
...
func (a *Agent) switchDashboard(target string) error {
	if target == "" {
...
func (a *Agent) downloadDashboard(ctx context.Context, request Request, work string, phase func(string)) (string, error) {
	...
	if err := writeJSON(filepath.Join(root, "ufi-dashboard.json"), DashboardStatus{Version: version}); err != nil {
		return "", err
	}
	phase("installing")
