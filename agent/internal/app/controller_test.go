...
func TestDeviceControllerOverridesSubscriptionWithoutSecret(t *testing.T) {
	...
	config, ports, err := adaptConfig(source, control, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	config, ports, err = adaptConfig(source, control, true)
...
func TestControllerSettingsCommitWithConfigAndRollbackOnFailure(t *testing.T) {
	...
	request := Request{ID: randomID(), Value: `{"enabled":true,"port":9191,"reset":true}`}
	...
	request.ID = randomID()
	request.Value = `{"enabled":false,"port":9292}`
	if _, err := a.saveController(context.Background(), request, func(string) {}); err == nil {
