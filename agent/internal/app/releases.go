...
func releaseVersion(tag string) (*semver.Version, error) {
	return semver.StrictNewVersion(strings.TrimPrefix(strings.TrimPrefix(tag, "agent-"), "v"))
}
...
func (a *Agent) latestRelease(ctx context.Context, owner, repo string) (release, error) {
...
func (a *Agent) updateAgent(ctx context.Context, work string, phase func(string)) (string, error) {
	if a.running() {
		return "", errors.New("请先停止代理")
	}
	phase("release")
	r, err := a.latestRelease(ctx, "imbytecat", "ufi-mihomo")
	if err != nil {
		return "", err
	}
	...
	_, address, digest, err := r.asset("imbytecat/ufi-mihomo", "mihomo-agent-linux-"+arch)
	if err != nil {
		return "", err
	}
