...
func TestAtomicWritesReplaceWithoutFollowingLinks(t *testing.T) {
	...
	defer old.Close()
	if err := atomicWrite(target, []byte("new"), 0700); err != nil {
		t.Fatal(err)
	}
	...
	if err := atomicWrite(link, []byte("secret"), 0600); err != nil {
		t.Fatal(err)
	}
	...
	if err := atomicWrite(dir, []byte("failure"), 0600); err == nil {
...
func TestProcessIdentityRejectsStalePID(t *testing.T) {
	...
	for _, recorded := range []string{start, "stale"} {
		if err := writeJSON(a.runtime("core.json"), processRecord{os.Getpid(), recorded}); err != nil {
			t.Fatal(err)
		}
		process, alive := a.process("core")
		if process != nil {
			process.Release()
		}
