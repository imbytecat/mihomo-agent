...
for (const [name, goarch, goarm] of [['arm64', 'arm64', ''], ['armv7', 'arm', '7']]) {
  ...
  const child = Bun.spawn(['go', 'build', '-trimpath', '-buildvcs=false', '-ldflags', `-s -w -buildid= -X main.version=${version}`, '-o', `../.release/${file}`, '.'], {
    cwd: 'agent', env: { ...buildEnv, GOOS: 'linux', GOARCH: goarch!, GOARM: goarm!, GOARM64: 'v8.0' }, stdout: 'inherit', stderr: 'inherit',
  });
...
await writeFile('agent-bootstrap.json', JSON.stringify({ version, protocol: 1, assets }, null, 2) + '\n');
